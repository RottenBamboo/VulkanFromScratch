...
namespace RottenBamboo{
    ...
    template <int ImageCount, int BufferCount>
    void RBDescriptors<ImageCount, BufferCount>::createDescriptorSetsFrameBuffer(std::array<VkImageLayout, TEXTURE_PATHS_MECH_GBUFFER_OUTPUT_COUNT> lightingImageLayouts)
    {
        std::vector<VkDescriptorSetLayout> layouts(MAX_FRAMES_IN_FLIGHT, descriptorSetManager.descriptorSetLayoutManager.descriptorSetLayout);
        descriptorSetManager.fillDescriptorSetsAllocateInfo(descriptorSetManager.descriptorPoolManager.descriptorPool, MAX_FRAMES_IN_FLIGHT, layouts.data());
        descriptorSetManager.allocateDescriptorSets(rbDevice, MAX_FRAMES_IN_FLIGHT);
    ...
    void RBDescriptors<ImageCount, BufferCount>::createTextureImage()
    {
        ...
        for (auto & imageBundle : rbImageManager.imageBundles)
        {
            ...
            float* pixels = stbi_loadf(paths[index].c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
            ...
            rbImageManager.fillImageInfo(texWidth, texHeight, mipLevels, msaaSamples, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_TILING_OPTIMAL, usageFlags);
            ...
            rbImageManager.transitionImageLayout(commandBuffer, imageBundle.image, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);
            ...
            //rbImageManager.transitionImageLayout(commandBufferEnd, imageBundle.image, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, mipLevels);
            //rbCommandBuffer.endSingleTimeCommands(commandBufferEnd);

            generateMipmaps(imageBundle.image, VK_FORMAT_R32G32B32A32_SFLOAT, texWidth, texHeight, mipLevels);
            index++;
    ...
    void RBDescriptors<ImageCount, BufferCount>::createTextureImageView()
    {
        for (auto & imageBundle : rbImageManager.imageBundles)
        {
            rbImageManager.fillViewInfo(imageBundle.viewInfo, imageBundle.image, VK_IMAGE_VIEW_TYPE_2D, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
    ...
    template<int ImageCount, int BufferCount>
    RBDescriptors<ImageCount, BufferCount>::RBDescriptors(RBDevice& device, RBCommandBuffer& commandBuffer, RBBuffer<UniformBufferShaderVariables> *uniformBuffers, const std::array<std::string, ImageCount> &texturePaths, const std::array<ImageInfo, ImageCount> &imageInfos, bool isColorAttachment) : rbDevice(device),
    ...
    template<int ImageCount, int BufferCount>
    RBDescriptors<ImageCount, BufferCount>::RBDescriptors(RBDevice& device, RBCommandBuffer& commandBuffer, RBBuffer<UniformBufferShaderVariables> *uniformBuffers, bool isColorAttachment) : rbDevice(device),
    rbCommandBuffer(commandBuffer),
    rbBufferPtr(uniformBuffers),
    descriptorSetManager(device),
    rbImageManager(device),
